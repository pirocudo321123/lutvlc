...
#include "VlcMsvcCompat.h" // MUST be first - defines ssize_t/poll/N_ for the VLC headers
...
namespace
{
    ...
    // Named lut_hdr_sys_t, not filter_sys_t, on purpose. VLC 3.0's vlc_common.h
    // already forward-declares `struct filter_sys_t` at global scope so filter_t
    // can hold a `filter_sys_t *p_sys`. Defining our own filter_sys_t in here
    // (anonymous namespace) does NOT complete that type - it creates a second,
    // unrelated one, and the assignment to p_sys then fails to compile. Modules
    // written in C get away with the name because there the local definition
    // completes the declared type; C++ namespacing breaks that. Distinct name +
    // an explicit cast at the two p_sys boundaries is the clean fix.
    struct lut_hdr_sys_t
    {
    ...
    void EnsureLutLoaded(lut_hdr_sys_t* sys)
    {
    ...
    const Lut3D& SelectCpuLut(lut_hdr_sys_t* sys, SourceColorSpace space)
    {
        switch (space)
        {
    ...
    {
        auto* sys = reinterpret_cast<lut_hdr_sys_t*>(p_filter->p_sys);
        ...
        // i_visible_* are unsigned; everything downstream (YuvOps, PixelOps,
        // D3D11LutEngine) takes int. Cast once here rather than at each call.
        const int width = static_cast<int>(p_pic->format.i_visible_width);
        const int height = static_cast<int>(p_pic->format.i_visible_height);
        ...
        if (chroma == VLC_CODEC_I420_10L)
        {
            // CPU-only for now, same as NV12 below - no GPU path exists yet for
            // planar 10-bit (D3D11LutEngine::ProcessFrameP010 only knows the
            // semi-planar layout). Given this chroma is what actually shows up
            // for HDR sources in practice (see the Open() comment above), that's
            // a real perf gap worth revisiting, not a correctness one.
            CopyPicturePlanes(p_outpic, p_pic);
            ProcessFrameI420_10(
                reinterpret_cast<uint16_t*>(p_outpic->p[0].p_pixels),
                reinterpret_cast<uint16_t*>(p_outpic->p[1].p_pixels),
                reinterpret_cast<uint16_t*>(p_outpic->p[2].p_pixels),
                width, height,
                p_outpic->p[0].i_pitch, p_outpic->p[1].i_pitch, p_outpic->p[2].i_pitch,
                effectiveColorInfo, sys->toneMap, SelectCpuLut(sys, effectiveColorInfo.space));
            picture_Release(p_pic);
            return p_outpic;
        }
    ...
    {
        auto* sys = reinterpret_cast<lut_hdr_sys_t*>(data);
    ...
    {
        auto* sys = reinterpret_cast<lut_hdr_sys_t*>(data);
        sys->lutOverride.store(static_cast<int>(newval.i_int), std::memory_order_relaxed);
        return VLC_SUCCESS;
    }
    ...
    int Open(vlc_object_t* p_this)
    {
        ...
        const vlc_fourcc_t chroma = p_filter->fmt_in.video.i_chroma;
        // VLC_CODEC_I420_10L: planar 10-bit 4:2:0 (ffmpeg's yuv420p10le layout).
        // This, not VLC_CODEC_P010, is what VLC's D3D11 opaque-surface readback
        // ("d3d11_filters") and software HEVC 10-bit decode both actually produce -
        // see ProcessFrameI420_10's header comment in YuvOps.h for why.
        ...
        auto* sys = new (std::nothrow) lut_hdr_sys_t();
        if (!sys)
            return VLC_ENOMEM;
        ...
        // reinterpret_cast, not a plain assignment: see the note on lut_hdr_sys_t.
        // filter_sys_t is incomplete here, which is fine - we only ever round-trip
        // the pointer, never dereference it as that type.
        p_filter->p_sys = reinterpret_cast<filter_sys_t*>(sys);
...
vlc_module_begin()
    ...
    add_loadfile(CFG_PREFIX "lut709", nullptr, LUT709_TEXT, LUT709_TEXT, false)
    add_loadfile(CFG_PREFIX "lut2020", nullptr, LUT2020_TEXT, LUT2020_TEXT, false)
    add_loadfile(CFG_PREFIX "lutdcip3", nullptr, LUTDCIP3_TEXT, LUTDCIP3_TEXT, false)
