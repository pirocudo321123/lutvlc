...
bool D3D11LutEngine::Initialize()
{
    ...
    // Log which GPU this device landed on. On hybrid-graphics laptops the default adapter
    // (nullptr above) can be the slow integrated GPU rather than the discrete one, which
    // directly changes the "gpuWait" number in the per-frame timing log.
    {
        ...
        if (SUCCEEDED(m_device.As(&dxgiDevice)) && SUCCEEDED(dxgiDevice->GetAdapter(&adapter)))
        {
            ...
            if (SUCCEEDED(adapter->GetDesc(&desc)))
                LutHdrTraceGpu(L"Initialize: adapter='%s' dedicatedVRAM=%llu MB", desc.Description,
...
namespace
{
    double NowMs()
    {
        ...
        return static_cast<double>(c.QuadPart) * invFreq;
    }

    // Copies `rows` rows of `rowBytes` bytes between two pitched buffers, split into
    // horizontal bands across the PPL thread pool. A single-threaded memcpy of a 4K
    // P010 frame (~25 MB) is memory-bandwidth-bound on one core; splitting it lets
    // several cores pull on the memory controller at once. Called four times per frame
    // (Y/UV upload, Y/UV readback), so this is where a good chunk of the CPU time was.
    void CopyRowsParallel(uint8_t* dst, size_t dstPitch,
                          const uint8_t* src, size_t srcPitch,
                          size_t rowBytes, int rows)
    {
        if (rows <= 0) return;
        const bool contiguous = (dstPitch == rowBytes && srcPitch == rowBytes);
        constexpr int kBands = 8;

        concurrency::parallel_for(0, kBands, [&](int band)
        {
            const int y0 = static_cast<int>(static_cast<long long>(rows) * band / kBands);
            const int y1 = static_cast<int>(static_cast<long long>(rows) * (band + 1) / kBands);
            if (y1 <= y0) return;

            if (contiguous)
            {
                memcpy(dst + static_cast<size_t>(y0) * rowBytes,
                       src + static_cast<size_t>(y0) * rowBytes,
                       static_cast<size_t>(y1 - y0) * rowBytes);
            }
            else
            {
                for (int y = y0; y < y1; ++y)
                    memcpy(dst + static_cast<size_t>(y) * dstPitch,
                           src + static_cast<size_t>(y) * srcPitch, rowBytes);
            }
        });
    }
}
...
bool D3D11LutEngine::EnsureTextures(int width, int height)
{
    ...
    auto make = [&](UINT w, UINT h, DXGI_FORMAT fmt, D3D11_USAGE usage, UINT bind, UINT cpu,
                    ComPtr<ID3D11Texture2D>& tex) -> bool
    {
        ...
        return SUCCEEDED(m_device->CreateTexture2D(&d, nullptr, &tex));
    };
    ...
    // Inputs are now D3D11_USAGE_DYNAMIC and filled with Map(WRITE_DISCARD) instead of
    // UpdateSubresource. UpdateSubresource on a DEFAULT texture makes the driver copy the
    // whole 25 MB frame into a hidden staging area first (an extra full-frame memcpy on
    // the streaming thread) before the GPU copy. Mapping a dynamic texture lets us write
    // straight into the driver's upload memory, once.
    if (!make(w,  h,  DXGI_FORMAT_R16_UINT,      D3D11_USAGE_DYNAMIC, D3D11_BIND_SHADER_RESOURCE,  D3D11_CPU_ACCESS_WRITE, m_texY_In))  return false;
    if (!make(uw, uh, DXGI_FORMAT_R16G16_UINT,   D3D11_USAGE_DYNAMIC, D3D11_BIND_SHADER_RESOURCE,  D3D11_CPU_ACCESS_WRITE, m_texUV_In)) return false;
    if (FAILED(m_device->CreateShaderResourceView(m_texY_In.Get(),  nullptr, &m_srvY_In)))  return false;
    if (FAILED(m_device->CreateShaderResourceView(m_texUV_In.Get(), nullptr, &m_srvUV_In))) return false;

    if (!make(w,  h,  DXGI_FORMAT_R16_UINT,    D3D11_USAGE_DEFAULT, D3D11_BIND_UNORDERED_ACCESS, 0, m_texY_Out))  return false;
    if (!make(uw, uh, DXGI_FORMAT_R16G16_UINT, D3D11_USAGE_DEFAULT, D3D11_BIND_UNORDERED_ACCESS, 0, m_texUV_Out)) return false;
    if (FAILED(m_device->CreateUnorderedAccessView(m_texY_Out.Get(),  nullptr, &m_uavY_Out)))  return false;
    if (FAILED(m_device->CreateUnorderedAccessView(m_texUV_Out.Get(), nullptr, &m_uavUV_Out))) return false;

    if (!make(w,  h,  DXGI_FORMAT_R16_UINT,    D3D11_USAGE_STAGING, 0, D3D11_CPU_ACCESS_READ, m_texY_Staging))  return false;
    if (!make(uw, uh, DXGI_FORMAT_R16G16_UINT, D3D11_USAGE_STAGING, 0, D3D11_CPU_ACCESS_READ, m_texUV_Staging)) return false;
    ...
    const DetectedColorInfo& colorInfo)
{
    ...
    // 4:2:0 with odd dimensions would make the UV plane's row/column count ambiguous;
    // real video is even. Fall back to the caller's passthrough copy rather than guess.
    if (width <= 0 || height <= 0 || ((width | height) & 1)) return false;

    if (!EnsureTextures(width, height)) return false;

    const double tStart = NowMs();

    const int uvHeight = height / 2;
    const size_t rowBytes = static_cast<size_t>(width) * 2; // Y: 2 bytes/sample. UV: width/2 texels * 4 bytes.
    ...
    // --- Upload -------------------------------------------------------------------
    D3D11_MAPPED_SUBRESOURCE mapIn{};
    if (FAILED(m_context->Map(m_texY_In.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapIn)))
        return false;
    CopyRowsParallel(static_cast<uint8_t*>(mapIn.pData), mapIn.RowPitch,
                     srcY, static_cast<size_t>(srcStrideBytes), rowBytes, height);
    m_context->Unmap(m_texY_In.Get(), 0);

    if (FAILED(m_context->Map(m_texUV_In.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapIn)))
        return false;
    CopyRowsParallel(static_cast<uint8_t*>(mapIn.pData), mapIn.RowPitch,
                     srcUV, static_cast<size_t>(srcStrideBytes), rowBytes, uvHeight);
    m_context->Unmap(m_texUV_In.Get(), 0);
    ...
    if (FAILED(m_context->Map(m_constantBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapCB)))
        return false;
    ...
    // Queue BOTH readback copies now, before touching the CPU side of either one.
    // Previously this was CopyResource(Y) -> Map(Y) -> memcpy -> CopyResource(UV) ->
    // Map(UV), which drained the GPU queue twice per frame and left it idle while the
    // CPU memcpy'd the Y plane. Now the GPU runs upload->compute->copy(Y)->copy(UV)
    // back to back and the CPU blocks exactly once.
    m_context->CopyResource(m_texY_Staging.Get(),  m_texY_Out.Get());
    m_context->CopyResource(m_texUV_Staging.Get(), m_texUV_Out.Get());

    const double tSubmitted = NowMs();
    ...
    // This Map is the one real sync point: it returns when the GPU has finished
    // everything queued above.
    if (FAILED(m_context->Map(m_texY_Staging.Get(), 0, D3D11_MAP_READ, 0, &mapY)))
        return false;
    const double tGpuDone = NowMs();

    CopyRowsParallel(dstY, static_cast<size_t>(dstStrideBytes),
                     static_cast<const uint8_t*>(mapY.pData), mapY.RowPitch, rowBytes, height);
    m_context->Unmap(m_texY_Staging.Get(), 0);

    if (FAILED(m_context->Map(m_texUV_Staging.Get(), 0, D3D11_MAP_READ, 0, &mapUV)))
        return false;
    CopyRowsParallel(dstUV, static_cast<size_t>(dstStrideBytes),
                     static_cast<const uint8_t*>(mapUV.pData), mapUV.RowPitch, rowBytes, uvHeight);
    m_context->Unmap(m_texUV_Staging.Get(), 0);

    const double tEnd = NowMs();

    // Rolling average, logged every 120 frames (~2s at 60fps). Read in DebugView.
    m_statUploadMs  += tSubmitted - tStart;   // upload memcpys + constants + dispatch submit
    m_statGpuWaitMs += tGpuDone - tSubmitted; // CPU blocked waiting on GPU (compute + DMA both ways)
    m_statCopyMs    += tEnd - tGpuDone;       // readback memcpys into the output buffer
    if (++m_statFrames >= 120)
    {
        const double n = static_cast<double>(m_statFrames);
        LutHdrTraceGpu(L"ProcessFrameP010 avg over %d frames: upload=%.2fms gpuWait=%.2fms readbackCopy=%.2fms total=%.2fms",
                       m_statFrames, m_statUploadMs / n, m_statGpuWaitMs / n, m_statCopyMs / n,
                       (m_statUploadMs + m_statGpuWaitMs + m_statCopyMs) / n);
        m_statUploadMs = m_statGpuWaitMs = m_statCopyMs = 0.0;
        m_statFrames = 0;
    }
